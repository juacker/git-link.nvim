...
describe("remote ref selection", function()
	...
	it("uses the closest remote branch when that branch has advanced", function()
		local repo = create_repo()
		run("git checkout -b v1.2.x main", repo.worktree)
		vim.fn.writefile({ "line one", "release change" }, repo.worktree .. "/file.lua")
		run("git add file.lua", repo.worktree)
		...
		local other_worktree = vim.fn.tempname()
		table.insert(temp_dirs, other_worktree)
		run("git clone " .. vim.fn.shellescape(repo.origin) .. " " .. vim.fn.shellescape(other_worktree))
		run("git config user.email test@example.com", other_worktree)
		run("git config user.name Tester", other_worktree)
		run("git checkout v1.2.x", other_worktree)
		vim.fn.writefile({ "line one", "release change", "remote change" }, other_worktree .. "/file.lua")
		run("git add file.lua", other_worktree)
		run("git commit -m remote-release-change", other_worktree)
		run("git push origin v1.2.x", other_worktree)
		run("git fetch origin", repo.worktree)
